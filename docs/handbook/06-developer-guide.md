...
metaphor migration generate Invoice auditlog
metaphor migration run
...
(`auditlog` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed auditlog          # run Rust seeders
metaphor migration generate-seeds auditlog  # emit SQL seed files
```
